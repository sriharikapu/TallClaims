$.get("http://localhost:5000/getList", function(data){
  ...
  for(var x = 0; x < data.length; x += 3){
    temp = `<div class="row" style="width: 100%">`;
    for(var y = 0; y < 3 && x+y < data.length; y++){
      temp += `<div class="card col-sm" onclick=paperDetails("${data[x+y].id}")  style="width: 18rem; margin-left: 20px;">
              <div class="card-body">
              ...
              <div class="card-body">
                <!--<aclass="card-link">3 Approve</a>-->
                <!--<a onclick=paperDetails("${data[x+y].id}") class="card-link">4 Unapprove</a>-->
                <a class="card-link">3 Approve</a>
                <a class="card-link">4 Unapprove</a>
              </div>
            </div>`;
    }
    temp += "</div>";
